...
const items = [
  { title: 'Сталь нержавеющая в рулонах и листах', route: '/steel-rolls' },
  { title: 'Сталь нержавеющая декоративная', route: '/decorative-steel' },
  { title: 'Окрашенная сталь нержавеющая', route: '/painted-steel' },
  { title: 'Сталь оцинкованная с полимерным покрытием', route: '/galvanized-steel' },
  { title: 'Кровля', route: '/production' }
]

const images = [
  { img: stalnerjImg, route: '/steel-rolls' },
  { img: dekoratImg, route: '/decorative-steel' },
  { img: okrasstalImg, route: '/painted-steel' },
  { img: ocinkovImg, route: '/galvanized-steel' }
]
          ...
          <div className="space-y-2 sm:space-y-3">
            {items.map((item, i) => (
              <div 
                ...
                className="bg-gray-800/50 backdrop-blur-sm p-3 sm:p-5 rounded-xl hover:bg-gray-800/80 transition-all cursor-pointer group"
              >
          ...
          <div className="grid grid-cols-2 gap-2 sm:gap-3">
            {images.map((item, i) => (
              <div 
                key={i} 
                onClick={() => navigate(item.route)}
                className="overflow-hidden rounded-lg sm:rounded-xl aspect-video border border-gray-700 cursor-pointer hover:border-gray-500 transition-colors"
              >
                <img src={item.img} alt="" loading="lazy" className="w-full h-full object-cover hover:scale-105 transition-transform duration-300" />
              </div>
            ))}
          </div>
        </div>
